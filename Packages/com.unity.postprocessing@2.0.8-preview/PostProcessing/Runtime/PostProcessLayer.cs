...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class PostProcessLayer : MonoBehaviour
    {
		...
		// 4.組み込み最終パス
		//
		// HDRディスプレイに出力するときは、最後のパスをスキップする必要があります。
		public void Render(PostProcessRenderContext context)
        {
			...
			// 不透明部分のみが通過する場合は、最初にレイヤー設定を更新して上書きする（ボリュームブレンディング）
			// このフレームとは呼ばれていません。
			UpdateSettingsIfNeeded(context);

			// Do a NaN killing pass if needed	必要ならばNaNキリングパスをする
			int lastTarget = -1;
			...
			//Do temporal anti - aliasing first 一時的なアンチエイリアシング(TAA)を最初に行う
			if (context.IsTemporalAntialiasingActive())
			{
				if (!RuntimeUtilities.scriptableRenderPipelineActive)
				{
					if (context.stereoActive) //VR用
					{
						// We only need to configure all of this once for stereo, during OnPreCull
						// OnPreCullの間に、これらすべてをステレオ用に1回だけ設定する必要があります。
			...
			// bloomの追加
			var bloom = context.camera.GetComponent<Bloom2D>();
			if (bloom != null && context.camera.cameraType == CameraType.Game)
			{
				var bloomTarget = m_TargetPool.Get();
				var finalDestination = context.destination;
				context.GetScreenSpaceTemporaryRT(cmd, bloomTarget, 0, context.sourceFormat);

				context.destination = bloomTarget;
				bloom.Render(context);
				context.source = bloomTarget;
				context.destination = finalDestination;

				if (lastTarget > -1)
					cmd.ReleaseTemporaryRT(lastTarget);

				lastTarget = bloomTarget;
			}

            // 新・グリッチ
            var glitch = context.camera.GetComponent<Glitch>();
            if (glitch != null && context.camera.cameraType == CameraType.Game)
            {
                var glichTarget = m_TargetPool.Get();
                var finalDestination = context.destination;
                context.GetScreenSpaceTemporaryRT(cmd, glichTarget, 0, context.sourceFormat);

                context.destination = glichTarget;
                glitch.Render(context);

                context.source = glichTarget;
                context.destination = finalDestination;

                if (lastTarget > -1)
                    cmd.ReleaseTemporaryRT(lastTarget);

                lastTarget = glichTarget;
            }

            //// グリッチエフェクトの追加
            //var glitch = context.camera.GetComponent<GlitchEffect>();
            //if (glitch != null)
            //{
            //	var glichTarget = m_TargetPool.Get();
            //	var finalDestination = context.destination;
        ...
        int RenderBuiltins(PostProcessRenderContext context, bool isFinalPass, int releaseTargetAfterUse = -1)
        {
			...
			if (!isFinalPass)
			{
				// Render to an intermediate target as this won't be the final pass	これは最終パスにはならないので、中間ターゲットにレンダリングします。

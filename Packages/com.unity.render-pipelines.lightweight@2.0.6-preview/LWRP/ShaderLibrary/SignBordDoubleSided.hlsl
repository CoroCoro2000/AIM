...
Texture2D _BackColorMap;
// Used for StandardSimpleLighting shader
half4 LitPassFragmentSimple(LightweightVertexOutput IN, bool facing : SV_IsFrontFace) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(IN);

    float2 uv = IN.uv;

    half4 diffuseAlpha = (facing) ? SampleAlbedoAlpha(uv, TEXTURE2D_PARAM(_MainTex, sampler_MainTex)) : SampleAlbedoAlpha(uv, TEXTURE2D_PARAM(_BackColorMap, sampler_MainTex));

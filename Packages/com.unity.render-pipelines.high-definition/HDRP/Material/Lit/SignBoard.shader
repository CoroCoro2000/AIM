/*====================================================*/
// 内容		：標識用テクスチャUV切り替えシェーダー
// ファイル	：SignBord.shader
//
// Copyright (C) 根本 勇斗 All Rights Reserved.
/*----------------------------------------------------*/
//〔更新履歴〕
// 2019/1/04 〔根本 勇斗〕新規作成
// 2019/1/07 〔根本 勇斗〕マテリアルプロパティブロックでuvをずらす処理に変更
// 2019/1/23 〔根本 勇斗〕GPUインスタンシング用にプロパティの値を設定
// 2019/2/12 〔根本 勇斗〕裏面も描画する処理を追加
/*====================================================*/
Shader "HDRenderPipeline/SignBoard"
{
	Properties
	{
...
#include "../../ShaderPass/VertMesh.hlsl"
	...
	UNITY_DEFINE_INSTANCED_PROP(float4,_UVColorInfo)
	UNITY_INSTANCING_BUFFER_END(unity_SignBord)
	...
	void Frag(PackedVaryingsToPS packedInput,bool facing:SV_IsFrontFace,
	...
	)
	{
		...
		_BaseColorMap_ST = float4(UNITY_ACCESS_INSTANCED_PROP(unity_SignBord, _UVColorInfo).r, UNITY_ACCESS_INSTANCED_PROP(unity_SignBord, _UVColorInfo).g,
			UNITY_ACCESS_INSTANCED_PROP(unity_SignBord, _UVColorInfo).b, UNITY_ACCESS_INSTANCED_PROP(unity_SignBord, _UVColorInfo).a);

		if (!facing)
			_BaseColorMap_ST = float4(0.2f, 0.2f, 0.4f, 0.4f);

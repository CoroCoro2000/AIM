Shader "HDRenderPipeline/LitVertTexBlender"
{
	Properties
	{
		...
		_PaintTex1("PaintTex1", 2D) = "white" {}
		_PaintTex2("PaintTex2", 2D) = "white" {}
		_PaintTex3("PaintTex3", 2D) = "white" {}
		_PaintTex4("PaintTex0", 2D) = "white" {}
		_PaintTex5("PaintTex1", 2D) = "white" {}
		_PaintTex6("PaintTex2", 2D) = "white" {}
		_PaintTex7("PaintTex3", 2D) = "white" {}
		_PaintTex8("PaintTex0", 2D) = "white" {}
		_PaintTex9("PaintTex1", 2D) = "white" {}
		_PaintTex10("PaintTex2", 2D) = "white" {}
		_PaintTex11("PaintTex3", 2D) = "white" {}
		_PaintTex12("PaintTex0", 2D) = "white" {}
		_PaintTex13("PaintTex1", 2D) = "white" {}
		_PaintTex14("PaintTex2", 2D) = "white" {}
		_PaintTex15("PaintTex3", 2D) = "white" {}
		_PaintTex16("PaintTex0", 2D) = "white" {}

		_PaintDataTexture("PaintDataTexture", 2D) = "white" {}
...
#pragma multi_compile _ SHOWPAINTINGCOLOR_ON
...
#include "../../ShaderPass/VertMesh.hlsl"
		...
		Texture2D _PaintTex1;
		Texture2D _PaintTex2;
		Texture2D _PaintTex3;
		Texture2D _PaintTex4;
		Texture2D _PaintTex5;
		Texture2D _PaintTex6;
		Texture2D _PaintTex7;
		Texture2D _PaintTex8;
		Texture2D _PaintTex9;
		Texture2D _PaintTex10;
		Texture2D _PaintTex11;
		Texture2D _PaintTex12;
		Texture2D _PaintTex13;
		Texture2D _PaintTex14;
		Texture2D _PaintTex15;
		Texture2D _PaintTex16;

		Texture2D _PaintDataTexture;
		SamplerState sampler_PaintDataTexture;
		float4 _PaintDataTexture_TexelSize;
		...
		)
		{
			...
			LayerTexCoord tex;	//UV構造体
			ZERO_INITIALIZE(LayerTexCoord, tex);//構造体初期化
			GetLayerTexCoord(input, tex);//情報取得、入れ込み

			float4 tex1 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex1, sampler_BaseColorMap, tex.base);
			float4 tex2 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex2, sampler_BaseColorMap, tex.base);
			float4 tex3 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex3, sampler_BaseColorMap, tex.base);
			float4 tex4 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex4, sampler_BaseColorMap, tex.base);
			float4 tex5 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex5, sampler_BaseColorMap, tex.base);
			float4 tex6 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex6, sampler_BaseColorMap, tex.base);
			float4 tex7 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex7, sampler_BaseColorMap, tex.base);
			float4 tex8 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex8, sampler_BaseColorMap, tex.base);
			float4 tex9 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex9, sampler_BaseColorMap, tex.base);
			float4 tex10 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex10, sampler_BaseColorMap, tex.base);
			float4 tex11 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex11, sampler_BaseColorMap, tex.base);
			float4 tex12 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex12, sampler_BaseColorMap, tex.base);
			float4 tex13 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex13, sampler_BaseColorMap, tex.base);
			float4 tex14 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex14, sampler_BaseColorMap, tex.base);
			float4 tex15 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex15, sampler_BaseColorMap, tex.base);
			float4 tex16 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintTex16, sampler_BaseColorMap, tex.base);

			float2 uv0 = (float2(input.color.x, input.color.y)+0.5)*_PaintDataTexture_TexelSize.xy;

			int a1 = (int)(input.color.b == 0 && input.color.a == 0);
			int a2 = (int)(input.color.b > 0.5 && input.color.a == 0);
			int a3 = (int)(input.color.b == 0 && input.color.a > 0.5);
			int a4 = (int)(input.color.b > 0.5 && input.color.a > 0.5);

			LayerTexCoord dummy = tex;
			dummy.base.uv = uv0;
			float4 data0 = SAMPLE_UVMAPPING_TEXTURE2D(_PaintDataTexture, sampler_PaintDataTexture, dummy.base);
			...
			surfaceData.baseColor =
				lerp(lerp(lerp(lerp(
					lerp(lerp(lerp(lerp(
						lerp(lerp(lerp(lerp(
							lerp(lerp(lerp(lerp(
								surfaceData.baseColor, tex1.rgb, data0.r*a1*tex1.a), tex2.rgb, data0.g*a1*tex2.a), tex3.rgb, data0.b*a1*tex3.a), tex4.rgb, data0.a*a1*tex4.a)
							, tex5.rgb, data0.r*a2*tex5.a), tex6.rgb, data0.g*a2*tex6.a), tex7.rgb, data0.b*a2*tex7.a), tex8.rgb, data0.a*a2*tex8.a)
						, tex9.rgb, data0.r*a3*tex9.a), tex10.rgb, data0.g*a3*tex10.a), tex11.rgb, data0.b*a3*tex11.a), tex12.rgb, data0.a*a3*tex12.a)
					, tex13.rgb, data0.r*a4*tex13.a), tex14.rgb, data0.g*a4*tex14.a), tex15.rgb, data0.b*a4*tex15.a), tex16.rgb, data0.a*a4*tex16.a)
				*ADD_IDX(_BaseColor).rgb;
				
//#ifdef SHOWPAINTINGCOLOR_ON
		//	surfaceData.baseColor = data0.rgb;
//#endif

...
// ==================================================================================================
Shader "Hidden/HDRenderPipeline/Sky/CloudSky"
{
    HLSLINCLUDE
    ...
    float4 Frag(Varyings input) : SV_Target
    {
        ...
        if(eyeRay.y >= 0.0)
        {
			...
			//--------------------------------------------------------------------------------------------------
			float3 skyColor_cube = ClampToFloat16Max(SAMPLE_TEXTURECUBE_LOD(_Cubemap, sampler_Cubemap, dir, 0).rgb * exp2(_SkyParam.x) * _SkyParam.y);	//min(max(x, a), b)
			//if (skyColor_cube.r >= 1.0f)
			
			skyColor = lerp(skyColor_cube,skyColor,0.95f);

			//skyColor = skyColor_cube;

			//skyColor = lerp(skyColor_cube.rgb, skyColor, step(skyColor_cube.r, 1.0f));
			//--------------------------------------------------------------------------------------------------
        }
        else
        {

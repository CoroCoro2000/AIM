...
{
	public class HDRenderPipeline : RenderPipeline  //RenderPipelineレンダリング部分を記述する 
	{
		enum ForwardPass
        {
            Opaque,
        ...
        public HDRenderPipeline(HDRenderPipelineAsset asset)
        {
            ...
            m_Asset = asset;    // readonlyはコンストラクタ内でも初期化できる

			// Initial state of the RTHandle system.
			// Tells the system that we will require MSAA or not so that we can avoid wasteful render texture allocation.
			// TODO: Might want to initialize to at least the window resolution to avoid un-necessary re-alloc in the player
			// RTHandleシステムの初期状態。
            // 無駄なレンダリングテクスチャの割り当てを回避できるように、MSAAが必要かどうかをシステムに指示します。
			// TODO:プレイヤーで不要な再割り当てを避けるため少なくともウィンドウの解像度を初期化したい場合があります
			...
			// 0以外のGバッファ数を持つ最初のマテリアルを探してdeferredMaterialとして割り当てます
			m_DeferredMaterial = null;
            foreach (var material in m_MaterialList)
            {
                if (material.GetMaterialGBufferCount() > 0)
                    m_DeferredMaterial = material;	// 中身確認したところどんな場合でも0でnullになる
            }

			// TODO: Handle the case of no Gbuffer material
			// TODO: I comment the assert here because m_DeferredMaterial for whatever reasons contain the correct class but with a "null" in the name instead of the real name and then trigger the assert
			// whereas it work. Don't know what is happening, DebugDisplay use the same code and name is correct there.
			// TODO: GBufferマテリアルがない時の取り扱い
			// TODO: m_DeferredMaterialは、なんらかの理由で正しいクラスを含みますが、実際の名前ではなく名前に「null」が含まれているため、ここでアサートをコメントしてからアサートをトリガします。
			// それは動作します。何が起こっているのかわからない、DebugDisplayは同じコードを使用し、名前はそこに正しいです。
			//Debug.Assert(m_DeferredMaterial != null);	// 初めからコメントアウトだった。使ってないっぽい
			...
			// 実行時にシーンビューのデバッグは必要ありません。（各カメラには独自のデバッグ設定があります）
        ...
        bool NeedDepthBufferCopy()
        {
			...
			// 今のところ、すべてのコンソールはバウンド深度バッファから読み取ることができると考えます。
        ...
        bool NeedStencilBufferCopy()
        {
			...
			// 現在、Unityはステンシルバッファをコンピュートシェーダのテクスチャとしてバインドする方法を提供していません。
			// したがって、ピクセルシェーダを使用して手動でコピーされます。
        ...
        public override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            {
				// SRP.Render() can be called several times per frame.						SRP.Render（）はフレームごとに複数回呼び出すことができます。
				// Also, most Time variables do not consistently update in the Scene View.	また、ほとんどの時間変数はシーンビューで一貫して更新されません。
				// This makes reliable detection of the start of the new frame VERY hard.	これは新しいフレームの開始の確実な検出を非常に困難にする。
				// One of the exceptions is 'Time.realtimeSinceStartup'.					例外の1つは「Time.realtimeSinceStartup」です。
				// Therefore, outside of the Play Mode we update the time at 60 fps,		したがって、プレイモード以外では、60 fpsで時間を更新します。
				// and in the Play Mode we rely on 'Time.frameCount'.						そしてプレイモードでは 'Time.frameCount'を使います。
				float t = Time.realtimeSinceStartup;
                uint  c = (uint)Time.frameCount;

                bool newFrame;

                if (Application.isPlaying) // 再生モードのとき Unity エディターでは true を返します。
                {
			...
			// さまざまなカメラで使用できるように、まずアセットフレーム設定の状態を更新します。
			// ただし、RenderingPath.Defaultを使用する他のカメラを正しくリセットするために、ダーティ状態を維持します
            ...
            foreach (var camera in cameras)
            {
                ...
                RenderPipeline.BeginCameraRendering(camera);    //1台のカメラがレンダリングを開始する前に、SRPレンダリング中に使用されたデリゲートを呼び出す

				// First, get aggregate of frame settings base on global settings, camera frame settings and debug settings
				// Note: the SceneView camera will never have additionalCameraData
				// まず、グローバル設定、カメラフレーム設定、およびデバッグ設定に基づいてフレーム設定の集計を取得します。
                // 注：SceneViewカメラは追加のCameraDataを持つことはありません。
				...
				// 各カメラの有効フレーム設定を初期化する
				// 各カメラには、デバッグウィンドウから独自のデバッグフレーム設定コントロールがあります。
				// デバッグフレーム設定をフレーム設定と統合することはできません（つまり、たとえば、制御のみを前方に集約することはできません）。
				// デバッグ設定（使用時）が有効なフレーム設定です
				// この動作を可能にするために、シリアル化されたフレーム設定で有効なフレーム設定を初期化してコピーします。
				// デバッグ設定はその上で変更されます。エディタでフレーム設定が変更されるたびに、すべてのデバッグ設定がリセットされます。
				// 同期を維持する以下のループは、すべてのフレーム設定を正しく更新することを可能にしています。
				// カメラはHDRendeRPipelineAssetのデフォルトのフレーム設定に依存できます
				...
				// グローバル設定とカメラタイプを考慮して、このカメラの有効フレーム設定を取得します
				...
				// これはフレームに使用されるメインコマンドバッファです。
				var cmd = CommandBufferPool.Get("");

				// Specific pass to simply display the content of the camera buffer if users have fill it themselves (like video player)
				// ユーザーが自分で埋めた場合は、カメラバッファの内容を単純に表示するための特定のパス（ビデオプレーヤーのように）
                ...
                if (camera.cameraType != CameraType.Reflection && camera.cameraType != CameraType.Preview
					...
					// 平面のReflectionProbeは正射投影に対応していない
					// エラーログのスパムを防ぐためにレンダリングを避けます
					&& !camera.orthographic)
                    // TODO: Render only visible probes
					// 可視プローブだけレンダリング
				...
				// 必要に応じてマテリアルを初期化する
				// TODO：これはカメラループの外側に移動する必要がありますが、コマンドバッファはありません。詳細をTimまたはJulienに依頼してください。
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
					...
					// 新しいフレームに対して必要なことをすべて行います。
                    m_LightLoop.NewFrame(currentFrameSettings);

					// If we render a reflection view or a preview we should not display any debug information	
					// This need to be call before ApplyDebugDisplaySettings()
					// リフレクションビューまたはプレビューをレンダリングする場合は、デバッグ情報を表示しないでください。
					// これはApplyDebugDisplaySettings（）の前に呼び出す必要があります
					if (camera.cameraType == CameraType.Reflection || camera.cameraType == CameraType.Preview)
                    {
						// Neutral allow to disable all debug settings	
						// すべてのデバッグ設定を無効にすることをニュートラルに許可
                    ...
                    using (new ProfilingSample(cmd, "Volume Update", CustomSamplerId.VolumeUpdate.GetSampler()))
                    {
                        ...
                        else
                        {
							...
							// 一時的なハック：
							// シーンビューでは、デフォルトで、存在する場合は「メイン」カメラボリュームレイヤーマスクを使用します
							// そうでなければ、競合を避けるために現在の空のライティングオーバーライドレイヤを削除するだけです。
							// これは任意のため、シーンビューで編集可能なはずです。
							if (camera.cameraType == CameraType.SceneView)
                            {
                                ...
                                if (needFallback)
                                {
									...
									// オーバーライドレイヤが "Everything"の場合、現在のレイヤがないことで問題を回避するために、現在のレイヤマスクの "Everything"にフォールバックします。
									// 実際には意味がないため、上書きマスクとして「Everything」を使用しないでください（UIで警告が発行されます）。
                                    if (m_Asset.renderPipelineSettings.lightLoopSettings.skyLightingOverrideLayerMask == -1)    // 符号付bit -1 => 1111 1111 全てのフラグが立っている状態
										layerMask = -1;
                                    else
					...
					// デバッグモードを有効にした場合、またはポストプロセスレイヤが無効になっている場合は、ポストプロセスを無効にします。
                    ...
                    ScriptableCullingParameters cullingParams;  // カリング用の設定
                    ...
                    if (hdCamera.frameSettings.enableDBuffer)
                    {
                        ...
                        // デカールシステムは現在のカメラで更新する必要があります。カリングとライトリスト生成パラメータを設定する必要があります。
                    ...
                    using (new ProfilingSample(cmd, "DBufferPrepareDrawData", CustomSamplerId.DBufferPrepareDrawData.GetSampler()))
                    {
                        if (hdCamera.frameSettings.enableDBuffer)
                        {
                            ...
                            m_DbufferManager.EnableDBUffer = true;              // mesh decals are renderers managed by c++ runtime and we have no way to query if any are visible, so set to true	メッシュデカールはc ++ランタイムによって管理されるレンダラーであり、表示されているものを照会する方法はないため、trueに設定します。
							DecalSystem.instance.UpdateCachedMaterialData();    // textures, alpha or fade distances could've changed																テクスチャ、アルファ、またはフェード距離が変更されている可能性があります
							DecalSystem.instance.CreateDrawData();              // prepare data is separate from draw																				データの作成は描画とは別のものです。
							DecalSystem.instance.UpdateTextureAtlas(cmd);       // as this is only used for transparent pass, would've been nice not to have to do this if no transparent renderers are visible, needs to happen after CreateDrawData	これは透過的なパスにのみ使用されるので、透過的なレンダラーが表示されていない場合はこれを実行しなくてもよいようにするためにはCreateDrawDataの後に実行する必要があります。
						}
                    }
					...
					// TODO：これらのマテリアルテクスチャをバインドするための正しい場所を見つける
					// このモードでは素材固有のグローバルパラメータをバインドする必要があります
					m_MaterialList.ForEach(material => material.Bind());

					// Frustum cull density volumes on the CPU. Can be performed as soon as the camera is set up.	
					// CPU上の錐台カル密度ボリューム。カメラをセットアップしたらすぐに実行できます。
					...
					// 注：ShadowMaskの場合、レガシーUnityはこのように動作します
					// ライティングパネルでShadowMaskを選択すると、SHADOW_MASKキーワードを使用してその場でシェーダを再コンパイルします。
					// ただし、Lighting Panelでどのモードが選択されているかを知るために問い合わせることができるC＃関数はなく、それはとにかく間違っているでしょう。照明パネルの設定次のベイクモードになります。しかし、光が当たるまでは、それは間違っています。
					// 現在シャドウマスクが必要かどうかを知るには、（CullResultの）すべての可視ライトを通過する必要があるので、LightBakingOutput構造体を調べて、lightmapBakeType / mixedLightingModeを調べます。 1つのライトにシャドウマスクベイクモードがある場合は、シャドウマスク機能（つまり追加のGバッファ）が必要です。
					// ベイクシャドウマスクを使用してライトを検出すると、スタンドアロンシャドウプレーヤーを構築するときにすべてのシェーダバリアントが生成され（実行時に）、ベイクシャドウマスクライトが表示されるときに動的に割り当てられます。追加のGBufferとシェーダを切り替えます。
					// 最初にやるべきことは、すべての光を通り抜けることです：PrepareLightsForGPU
					...
					// DBufferに必要な場合、これは深度バッファをバインドします。
					...
					// Normalのバッファをバインドして、あらゆる効果で使用できるようになりました。
                    m_NormalBufferManager.BindNormalBuffers(cmd);

					// In both forward and deferred, everything opaque should have been rendered at this point so we can safely copy the depth buffer for later processing.
					// 順方向でも据え置きでも、この時点で不透明なものはすべてレンダリングされているはずなので、後で処理するために深度バッファを安全にコピーできます。
					...
					// TODO：将来的には深度の準備と同時にオブジェクトの速度をレンダリングするでしょう（これにはC ++の修正が必要です）
					// C ++の変更が行われたら、まずモーションベクトルなしですべてのオブジェクトをレンダリングし、次にモーションベクトルオブジェクトをレンダリングします。
					// 深度の準備ができていない場合は拒否されるべきであるモーションベクトルの書き込みがある可能性があるため、現在、深度の準備の後にオブジェクトの速度をレンダリングすることはできません。
					RenderObjectsVelocity(m_CullResults, hdCamera, renderContext, cmd);
                    RenderCameraVelocity(m_CullResults, hdCamera, renderContext, cmd);

					// Depth texture is now ready, bind it (Depth buffer could have been bind before if DBuffer is enable)
					// 深度テクスチャの準備ができました。それをバインドします（DBufferが有効になっている場合、以前に深度バッファがバインドされていた可能性があります）。
					...
					// 注意：ここでは太陽光をレンダリングするために使用しているので太陽光が必要です。つまり、UpdateSkyEnvironmentはPrepareLightsForGPUの後に呼び出す必要があります。
                    // TODO：他のパスでスカイコンボリューションを実行するためにここでasync computeを使用できるようにコードを調整してください（コンボリューションシェーダを計算に移動した後）。
                    ...
                    else
                    {
                        ...
                        using (new ProfilingSample(cmd, "Render SSAO", CustomSamplerId.RenderSSAO.GetSampler()))
                        {
							...
							// TODO：すべて（SSAO、シャドウ、ビルドライトリスト、遅延シャドウ、マテリアル、およびライトの分類は、非同期計算で並列化できます）
						...
						// 非同期ライトリストビルドを呼び出す前にステンシルテクスチャをクリアしてコピーする必要があります。
						// そうでなければ、非同期計算キューはグラフィックスキューがそれを処理する前にそのテクスチャを使用することになる可能性があります。
						// TODO：このコードをLightLoop内に移動します
						if (m_LightLoop.GetFeatureVariantsEnabled())
                        {
							// For material classification we use compute shader and so can't read into the stencil, so prepare it.
							// マテリアルの分類にはコンピュートシェーダを使用しているのでステンシルに読み込むことはできませんので、準備してください。
							using (new ProfilingSample(cmd, "Clear and copy stencil texture", CustomSamplerId.ClearAndCopyStencilTexture.GetSampler()))
                            {
								...
								// マテリアル分類シェーダでは、テストしますが、照明ではありません。
								// ShaderPassIDを使用する1 =>「1を渡す - 値がstencilRefと異なる場合は1を書き込む」
						...
						//半透明用デプスプレパス-------
						var pass = new ShaderPassName("ZPrePass");					//シェーダパス名
						...
						renderContext.DrawRenderers(m_CullResults.visibleRenderers, ref settings, filterSettings);
						//-----------------------------
						...
						// Might float this higher if we enable stereo w/ deferred	ステレオw / deferredを有効にしている場合は、これよりも高く浮動する可能性があります。
						StartStereoRendering(renderContext, hdCamera);

						RenderForward(m_CullResults, hdCamera, renderContext, cmd, ForwardPass.Opaque);

						// SSS pass here handle both SSS material from deferred and forward		ここでSSSパスは、延期から順方向の両方のSSS品目を処理します。
						...
						//var sour = HDShaderIDs._CameraColorTexture;
				...
				// 注意：ExecuteCommandBufferはプロファイリングブラケットの外側にある必要があります
				renderContext.ExecuteCommandBuffer(cmd);
            ...
            )
        {
			...
			// This is done here because DrawRenderers API lives outside command buffers so we need to make call this before doing any DrawRenders
			// DrawRenderers APIはコマンドバッファーの外側にあるため、これはここで行われます。したがって、DrawRendersを行う前にこれを呼び出す必要があります。
			renderContext.ExecuteCommandBuffer(cmd);
            cmd.Clear();
		...
		// RenderGBufferはgbufferパスを行います。これはもっぱら延期された呼び出しです。もし我々が深さの準備を使用するならば、深さの準備はテストされた不透明なアルファについてアルファテストを実行するでしょう、そして、我々はもうそれをする必要はありません。
		// Gbufferパス中これはシェーダで処理され、ここで深度テスト（均等深度書き込みなし）が行われます。
        ...
        void RenderDBuffer(HDCamera hdCamera, CommandBuffer cmd, ScriptableRenderContext renderContext, CullResults cullResults)
        {
            ...
            {
				// We need to copy depth buffer texture if we want to bind it at this stage	この段階で深度バッファテクスチャをバインドする場合は、深度バッファテクスチャをコピーする必要があります。
				CopyDepthBufferIfNeeded(cmd);

				// Depth texture is now ready, bind it.	深さテクスチャは今準備ができて、それを結びます。
				cmd.SetGlobalTexture(HDShaderIDs._CameraDepthTexture, GetDepthTexture());
                m_DbufferManager.ClearTargets(cmd, hdCamera);
                HDUtils.SetRenderTarget(cmd, hdCamera, m_DbufferManager.GetBuffersRTI(), m_CameraDepthStencilBuffer); // do not clear anymore	もうクリアしないで
		...
		// 前方へのレンダリングは、透明オブジェクトと不透明オブジェクトの両方に使用されます。遅延が発生した場合でも、不透明なオブジェクトを前方にレンダリングできます。
        void RenderForward(CullResults cullResults, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd, ForwardPass pass)
        {
			...
			// ガイドライン：デフォルトで延期されているので、前方に不透明はありません。ただし、不透明な素材を前方にレンダリングするように強制することは可能です。
			// "ForwardOnly"パスを使用して。この場合、.shaderは "Forward"ではなく、 "ForwardOnly"パスのみを持つべきです。
			// 前方マテリアル（遅延レンダリングまたは前方レンダリングのみ）には常に深度パスがあるため、「DepthForwardOnly」パスも「DepthOnly」パスも設定しないでください。
			// RenderForwardパスはエンジン設定に応じて適切なパスをレンダリングします。順方向のみのレンダリングの場合、 "Forward"パスと "ForwardOnly"パスの両方
			// マテリアルは透明と不透明の両方に対してレンダリングされます。遅延の場合、両方のパスが透明に使用されますが、不透明には "ForwardOnly"のみが使用されます。
			//（したがって、 "Forward"と "ForwardOnly"が排他的である理由、それ以外の場合は2回レンダリングされる "
            ...
            using (new ProfilingSample(cmd, profileName, CustomSamplerId.ForwardPassName.GetSampler()))
            {
                ...
                if (pass == ForwardPass.Opaque)
                {
					...
					// フォワードSSSの場合は、必要なすべてのターゲットをバインドします。それに書くかどうかはシェーダ次第です。
                    if (hdCamera.frameSettings.enableSubsurfaceScattering)
                    {
        ...
        void RenderPostProcess(HDCamera hdcamera, CommandBuffer cmd, PostProcessLayer layer )
        {
			using (new ProfilingSample(cmd, "Post-processing", CustomSamplerId.PostProcessing.GetSampler()))
			{
				...
				// コンソールの場合、ウィンドウのサイズを変更することは許可されていないので、ハッキングを使用しない
				// ビューポートのサイズとレンダリングテクスチャのサイズが一致している場合もコピーは行いません。
				bool viewportAndRTSameSize = (hdcamera.actualWidth == m_CameraColorBuffer.rt.width && hdcamera.actualHeight == m_CameraColorBuffer.rt.height);  // カメラとレンダーテクスのサイズが同じか
				...
				if (tempHACK)
				{
					...
					// 一時的：
					// フルレンダーテクスチャにはレンダリングしないので、適切なスケール/バイアスで後処理スタックをフィードする必要があります。
					// この機能はまだ実装されていません。適切なサイズのRTに関連するバッファをコピーするだけです。
					...
					// GetTemporaryRT	一時的なレンダリングテクスチャを取得する
				...
				else
				{
					...
					// 注意：ここでは深度テクスチャを取得するためにGetDepthTexture（）を使用しませんが、Forward transparentパスとして使用できるm_CameraDepthStencilBuffer
					// DOF / MBを処理するための追加データを書き込む
		...
		public void ApplyDebugDisplaySettings(HDCamera hdCamera, CommandBuffer cmd)
        {
                ...
                m_CurrentDebugDisplaySettings.colorPickerDebugSettings.colorPickerMode != ColorPickerDebugMode.None)
            {
				...
				// マルチコンピューターでサポートするシェーダーでキーワードDEBUG_DISPLAYをグローバルに有効にします。
                cmd.EnableShaderKeyword("DEBUG_DISPLAY");

				// This is for texture streaming	これはテクスチャストリーミング用です
				...
				// DebugNeedsExposureテストでは、露出が不要な場合はニュートラル値を設定できます。このようにして、シェーダ内部でさまざまなテストを行う必要はなく、この関数だけでテストを行います。
            ...
            else
            {
				...
				// このキーワードの状態に変化がなければ、CPU側での作業を意味するものではありません。さもなければ私達はどこかに状況を保存する必要がある
